...
bool smulti_proxy(int argc, char ** argv)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            char buffer[1024];
            printf("socker:multi_proxy() about to call read()\n");
            ssize_t numBytes;
...
*/
                ...
                ONE_TIME_BUFFER_HANDLE dummy;
                while (getNextSession(si, &sdClient, &sdProxied, &dummy, &dummy))
                {
                    fromClientBuffer = createOneTimeBuffer((char *)"From Client");
                    toClientBuffer = createOneTimeBuffer((char * )"To Client");
                    ...
                    if (FD_ISSET(sdClient, &rfds))
                    {
                        ...
                        {
                            printf("socker:multi_proxy() about to print buffer from client\n");
                            printf("%s", buffer);
                            printf("\n");
                        ...
                        {
                            printf("socker:multi_proxy() read of zero bytes performed\n");
                    ...
                    if (FD_ISSET(sdProxied, &rfds))
                    {
                        ...
                        {
                            printf("socker:multi_proxy() about to print buffer from proxied\n");
                            printf("%s", buffer);
                            printf("\n");
                        ...
                        {
                            printf("socker:multi_proxy() proxied read of zero bytes performed\n");
                    ...
                    if (FD_ISSET(sdProxied, &wfds))
                    {
                        ...
                        {
                            printf("socker:multi_proxy() about to send to proxied\n");
                    ...
                    if (FD_ISSET(sdClient, &wfds))
                    {
                        ...
                        {
                            printf("socker:multi_proxy() about to send to client");
